...
void queueInit (tQueue* q)
{
    ...
    {
        for(int i = 0; i < QUEUE_SIZE ; i++) //pre všetky prvy v poli
        {
            q->arr[i]= '*'; //nastavime na hodnotu *
        }
        q->f_index = 0;  //index na začiatok fronty nastavíme na 0
        q->b_index = 0; //index prvého volného miesta nastavíme na 0
    }
}
...
{
    return(index + 1) % QUEUE_SIZE;     // modulo % vykonáva kruhový pohyb cez indexy pola
}
...
{
    return(q->f_index == q->b_index) ? 1 : 0;   // fronta je vtedy prázdná, keď sa index prvého prvku = indexu prvej volnej pozicie
}
...
{
    return(nextIndex(q->b_index) == q->f_index) ? 1 : 0; //fronta je plná vtedy keď sa posledný posunutý = prvému
}
...
void queueFront (const tQueue* q, char* c)
{
    if(queueEmpty(q) != 0)
    {
        ...
        *c = q->arr[q->f_index];  // pomocou c vrátime znak zo začiatku fronty
}
...
void queueRemove (tQueue* q)
{
    if(queueEmpty(q) != 0)
        ...
        q->f_index = nextIndex(q->f_index); // znak sa odstráni tak, že sa posunieme  pomocou pomocnej funkcie nextIndex
}
